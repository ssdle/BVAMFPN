...
class BVAM(nn.Module):
    ...
    def forward(self, x, H, W, relative_pos_index, relative_coords_table,kv):
    # def forward(self, x, H, W, relative_pos_index, relative_coords_table):
        ...
        #生成展开的键和值，并对其进行 L2 归一化
        k_local, v_local = self.kv(kv).chunk(2, dim=-1)
        # 改前，k_local, v_local = self.kv(x).chunk(2, dim=-1)
        ...
        #生成池化特征
        kv_ = kv.permute(0, 2, 1).reshape(B, -1, H, W).contiguous()
        kv_ = self.pool(self.act(self.sr(kv_))).reshape(B, -1, self.pool_len).permute(0, 2, 1)
        kv_ = self.norm(kv_)

        # Generate pooled keys and values
        #生成池化键和值
        kv_pool = self.kv(kv_).reshape(B, self.pool_len, 2 * self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        k_pool, v_pool = kv_pool.chunk(2, dim=1)

        # Use MLP to generate continuous relative positional bias for pooled features.
        #使用 MLP 为池化特征生成连续相对位置偏差。
        ...
        #连接本地和合并的相似性矩阵，并通过相同的Softmax计算注意力权重
